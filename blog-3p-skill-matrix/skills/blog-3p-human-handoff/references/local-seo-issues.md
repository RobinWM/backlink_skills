...
| Cross-language reader intent / regional-SERP localization | PASS / FINDING / WARN / N/A | Google Trends is optional English-only global relative-interest context. If it is `UNAVAILABLE` or `INCONCLUSIVE`, accept the documented target-platform audience-need, brand-site, and regional-SERP rationale with its evidence boundary; do not treat missing Trends alone as a finding. |
