...
`article-package.json@1.4` 是冻结声明与上游唯一来源的连接点：唯一基准稿、metadata、单一 evidence pack 和视觉清单。它不反向引用派生的 handoff manifest，也不手工重复 SEO 字段、链接或图片清单。唯一接受的交付页是一对由编译器生成的 `BLOG_3P_VISUAL_PAYLOAD@3`：`handoff/visual-payload.html` 供富文本复制，`handoff/visual-payload.md` 是完全对应的可读／追溯备用稿。W 提供 metadata 中的标题/SEO 字段、唯一基准稿正文/结构和 visual manifest；不得手写任一交付页、页面壳、CSS、JavaScript、按钮、卡片或操作顺序。
...
只有正文、metadata、链接、来源、读者价值承诺、冻结 CTA 和最终 visual manifest 都稳定后才编译。随后该文章既有 R 的 `FULL_REVIEW` 一次审单一 evidence pack、唯一基准稿、metadata、最终图片、视觉清单和 HTML／Markdown 交付页，检查图文邻接适配、每张图是否就在对应正文标记处、可读性、独立读者作用、覆盖区、Alt/图注、素材格式／命名、CTA 保真和固定模板一致性。它在 review index 中绑定所有最终文件哈希；默认标准路线同时绑定 evidence-pack 哈希，而不是额外创建研究审。
