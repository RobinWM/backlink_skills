...
New workspaces use schema `2.4`. The structural checker remains compatible with schemas `1.0` through `2.3`; legacy workspaces remain valid under their historical contract and must not be silently relabeled as schema 2.4. In particular, `cta.mode = NONE` is historical schema-2.1 compatibility only, never valid for a new schema-2.2+ campaign. Schema 2.3 added the structured public-return receipt, normalized read-only snapshot, per-article public-QA ledger and same-lane-G-only classification policy. Schema 2.4 adds canonical manifests, deterministic pre-write rendering, compact article/review context and bounded delta review.
...
- `cross_language_seo.google_trends_seed_language = ENGLISH_ONLY`
