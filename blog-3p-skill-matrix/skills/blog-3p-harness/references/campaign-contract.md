...
New workspaces use schema `2.13`. The structural checker may read schemas `1.0` through `2.12` under their historical contracts, but legacy workspaces must not be silently relabeled as schema 2.13 or used to start the current dispatch flow. Current schema 2.13 preserves the current human-native release, topic, shared-evidence, single-source-payload and R-primary-HTML contracts, while replacing the normal per-article worktree rule with `CONTINUOUS_CAMPAIGN_MAIN_SESSION` plus `MAIN_SESSION_PATH_ISOLATED`. Every article instead has a deterministic `articles/<article_id>/` artifact root; Git worktrees are recorded exceptions only. In particular, `cta.mode = NONE` remains historical schema-2.1 compatibility only, never valid for a new schema-2.2+ campaign.
...
- `orchestration_policy.writer_reviewer_pair_mode = REUSABLE_W_R_WITH_ARTICLE_ARTIFACT_ISOLATION`
- `orchestration_policy.cross_article_agent_reuse = CAMPAIGN_W_R_G_REUSE_ALLOWED`
...
- `orchestration_policy.article_worktree_role_bundle = REUSABLE_W_R_PLUS_SHARED_CAMPAIGN_G`
...
For a schema-1.5 `article-package.json`, every shorthand reference above to the final “visual payload” means the compiler-owned pair `handoff/visual-payload.html` and `handoff/visual-payload.md`, generated only from package-declared `canonical/article.html`, metadata, evidence pack and visual manifest. In schema 2.13 these all live below the same `articles/<article_id>/` root. A `FULL_REVIEW` binds both hashes but semantically reads HTML by default; Markdown joins the semantic review only when the validator emits `COMPANION_DUAL_READ_REQUIRED`. Each image must be represented by one ordered canonical-body `<!-- BLOG_3P_IMAGE:NN -->` marker and corresponding HTML/Markdown card at that position, with a hash-matched `01-lead-*`, `02-middle-*` or `03-closing-*` PNG/JPG/JPEG asset. A repeat render must be byte-identical for both files before handoff finalization. `1.4` is historical compatibility only.

## Current schema-2.13 execution and path-isolation contract

The normal dispatch creates one `article_workspaces[article_id]` record with `isolation: MAIN_SESSION_PATH_ISOLATED` and deterministic `artifact_root: articles/<article_id>`. Its `context`、`research`、`canonical`、`reviews` and `handoff` children must be disjoint from every other article. `CONTINUOUS_CAMPAIGN_MAIN_SESSION` permits the same visible W and R role sessions to handle ordinary articles in sequence or a safe path-separated batch. R remains an independent review role, uses each article's own contract/index/evidence, and cannot treat W's self-check or earlier article review as evidence. G remains the single campaign control plane.
...
### Current schema-2.13 release mapping and handoff rules
