...
New workspaces use schema `2.11`. The structural checker may read schemas `1.0` through `2.10` under their historical contracts, but legacy workspaces must not be silently relabeled as schema 2.11 or used to start the current dispatch flow. In particular, `cta.mode = NONE` is historical schema-2.1 compatibility only, never valid for a new schema-2.2+ campaign. Schema 2.3 added the structured public-return receipt, normalized read-only snapshot, per-article public-QA ledger and same-lane-G-only classification policy. Schema 2.4 added canonical manifests, deterministic pre-write rendering, compact article/review context and bounded delta review. Schema 2.5 locks optional English-only global Trends context and prohibited trend inferences without requiring a Trends artifact. Schema 2.6 makes model-continuous creation the default: one evidence pack, one independent final R review, and a separate research review only on a risk-elevated route. Schema 2.7 removes the redundant per-article G turn: one registered campaign G performs compact batch contract acceptance and batch public QA, while every article keeps its independent W/R pair and its own hash-bound evidence row. Schema 2.8 added receipt-bound owner confirmation and a distinct local dispatch-readiness report. Schema 2.9 removes the selectable local-only branch: `HUMAN_RELEASE_ONLY_V1` is the sole current profile, `release_policy.mode = HUMAN_NATIVE_ONLY`, platform writes remain disabled, and every article needs an owner-confirmed distinct platform/account and locale mapping before W/R dispatch. Schema 2.10 separates primary audience fit from editor/format transport, binds delivery/claim-posture scope snapshots to owner confirmation, and records platform-profile and empirical-claim evidence boundaries. Schema 2.11 adds the owner-confirmed `topic_slot`, its evidence-pack alignment record, and a single existing-G exception receipt for the rare case where evidence conflicts with the frozen topic boundary.

## Pre-write plan contract

G begins in `prewrite_planning`. Before any article worktree, W, R, article queue, outline, prose, image generation, canonical package or article-research role exists, G gathers and reports a pre-write dossier. It is a read-only research protocol and known-evidence summary, not a completed W research package; it must identify uncertainty and may not declare `RESEARCH_READY` or `RESEARCH_APPROVED`.

Canonical `prewrite-plan.json` and its generated owner-facing `prewrite-plan.md` must cover every configured `article_id` exactly once. In schema-2.11, the JSON manifest is schema `1.7`, includes the artifact convention, shared-evidence reference, frozen delivery mapping, `topic_slot`, evidence posture and a receipt-bound owner confirmation field, and is the only editable plan source; run `sync-prewrite-plan` after every plan change. The one campaign summary is `campaign_strategy`. Each plan contains non-empty `task_and_audience`, `reader_value_and_secondary_cta`, `editorial_brief`, and `risks_and_owner_decisions`, plus a valid `frozen_delivery_mapping`, `topic_slot`, `evidence_posture` and `review_effort`. The topic slot freezes reader task, core intent, market, differentiation angle and prohibited deviations without freezing literal keyword/title wording. The editorial brief is comprehensive but consolidated: it covers the proposed keyword/localization route, fact/source boundaries, title/outline, visual narrative and delivery assumptions without forcing duplicate evidence records. Its default review effort is `STANDARD_INTEGRATED_REVIEW / INTEGRATED_IN_FULL_REVIEW` with no reasons; `ELEVATED_EARLY_CHALLENGE / SEPARATE_RESEARCH_REVIEW_REQUIRED` needs one or more concrete risk reasons. Schema-2.2/2.3 retain manifest schema `1.2`; schema `2.4/2.5` retain schema `1.3`; schema `2.6/2.7` retain schema `1.4`; schema `2.8/2.9` retain schema `1.5`; schema `2.10` retains schema `1.6`; schema `2.1` uses manifest schema `1.1`; schema `2.0` uses manifest schema `1.0` and its historical section list. `UNVERIFIED` must say what was checked and the resulting risk; it is never a license to invent evidence.

G presents this dossier and moves to `awaiting_owner_prewrite_confirmation`. Only the explicit `OWNER_PREWRITE_PLAN_CONFIRMED` receipt permits article-lane provisioning. In schema 2.11, preserve that owner-originated receipt under `evidence/owner-confirmations/` and use `confirm-prewrite-plan`; it binds the owner confirmation ID, exact unique article IDs, report SHA-256, manifest SHA-256, protected scope snapshot/hash, receipt path/hash, locator and timestamp in `state.json.prewrite_plan`, `confirmation.md`, `requirements-contract.md`, `campaign.json.scope_lock.prewrite_plan_receipt`, and the report/manifest themselves. Before W/R dispatch, every article also needs an owner-confirmed distinct `{platform, account}` assignment and compatible audience/transport row. A change to article language, market, platform, account, fit mode, cross-language exception, `topic_slot` or evidence posture invalidates the binding; `sync-prewrite-plan` must return `SCOPE_RECONFIRM_REQUIRED` without overwriting the receipt. Use explicit invalidation to return to owner confirmation. `OWNER_PREWRITE_PLAN_CHANGES_REQUESTED` never authorizes a partial dispatch.

This does not create a second article-research role. After owner confirmation, W performs the complete integrated keyword, source and cross-language research. In the default route it then continues directly through draft, visuals and payload, and the same article's reusable R evaluates the entire evidence chain in one `FULL_REVIEW`. W runs `check-review-ready` immediately before that review to remove only path/hash/fixed-payload/CTA drift; its pass is not an editorial verdict. Only an elevated route requires R to issue `RESEARCH_APPROVED` before W outlines or drafts; it still ends with the mandatory full review. R may request elevation and G records the reason, but no role may lower an elevated route.
...
- `orchestration_policy.article_worktree_role_bundle = ONE_REUSABLE_W_R_PAIR_PLUS_SHARED_CAMPAIGN_G`
- `orchestration_policy.project_worktree_root_role = ARTICLE_WRITER_REVIEWER_PAIR`
...
- `artifact_optimization_policy.prewrite_manifest_schema = 1.7`
...
- `artifact_optimization_policy.review_route = STANDARD_INTEGRATED_REVIEW_UNLESS_ELEVATED`
- `model_first_execution_policy.mode = MODEL_CONTINUOUS_CREATION_WITH_RISK_ESCALATION`
- `model_first_execution_policy.default_review_tier = STANDARD_INTEGRATED_REVIEW`
- `model_first_execution_policy.separate_research_review = RISK_TRIGGERED_ONLY`
- `model_first_execution_policy.independent_final_review = REQUIRED`
...
For current schema 2.11, the preceding compatibility wording is narrowed: every article must additionally have an owner-confirmed primary-audience/transport row and protected scope snapshot. `PRIMARY_AUDIENCE_MATCH` is mandatory unless the selection receipt carries the exact `CROSS_LANGUAGE_EXCEPTION_OWNER_CONFIRMED` wording and ID. A transport-language observation, historical English content, or UI language cannot satisfy the primary-audience condition. The R `FULL_REVIEW` checks the single evidence pack's `topic_slot_alignment`, `reader_intent_basis`, bounded `platform_profile_use` and `evidence_posture`; this adds no default review or research task. An unresolved `TOPIC_EVIDENCE_CONFLICT` stops before full review; a resolved `KEEP_SLOT` / `NARROW_WITHIN_SLOT` conflict needs one matching `TOPIC_EVIDENCE_DECISION` in the registered G's existing visible task ledger.

For current `article-package.json@1.4` workspaces, every shorthand reference above to the final “visual payload” means the compiler-owned pair `handoff/visual-payload.html` and `handoff/visual-payload.md`. A `FULL_REVIEW` binds both hashes. Each image must be represented by one ordered canonical-body `<!-- BLOG_3P_IMAGE:NN -->` marker and corresponding HTML/Markdown card at that position, with a hash-matched `01-lead-*`, `02-middle-*` or `03-closing-*` PNG/JPG/JPEG asset. A repeat render must be byte-identical for both files before handoff finalization.
