...
运行 `scripts/harnessctl.py init --workspace <absolute-path> --campaign-id <id>`。它只创建本地文件：`campaign.json`、`state.json`、`confirmation.md`、`requirements-contract.md`、唯一可编辑的 `prewrite-plan.json`、确定性只读视图 `prewrite-plan.md`、一次性 `pre-clearance-checklist.md` 及标准证据/输出目录。当前 schema-2.13 工作区使用写前方案 schema `1.7`、evidence pack schema `1.3` 和 `article-package.json@1.5`，默认在持续内容项目主会话中把每篇交付物置于 `articles/<article_id>/{context,research,canonical,reviews,handoff}`，并创建 `evidence/owner-confirmations/` 保存用户原始确认回执；schema-2.12 / package-1.4 及更早版本仅保留读取和校验兼容。当前工作区只支持人工原生发布交接；脚本绝不执行平台写入。
...
2. `blog-3p-writer` 是唯一可执行 W；`blog-writer-merged` 只作编辑参考。默认路线中，W 连续完成研究、成稿、图片和交付页：共享证据只按精确引用进入本篇 evidence pack，`canonical/article.html`、metadata、evidence pack 与 visual manifest 由 `article-package.json@1.5` 单向声明，并由同一编译器生成 HTML/Markdown。R 一次独立 `FULL_REVIEW` 语义审 HTML 主投影及上游源；只有 `COMPANION_DUAL_READ_REQUIRED` 才双读 Markdown。只有冻结或 R 升级的增强路线才在正文前要求 `RESEARCH_APPROVED`；两种路线都必须有完整审稿。
3. `blog-3p-gate` 在一个 `BATCH_GATE_ACCEPTANCE` 回合中验收当前所有 R 已批准文章是否保留用户需求；每篇保留独立结论行。W 在 R 前已编译可视化富文本交付页；`blog-3p-human-handoff` 仅在该行 `HUMAN_RELEASE_READY` 后生成派生人工发布清单，不重新编译或改写已审载荷。
4. 人工回传 `HUMAN_ACCEPTED` URL 后，校验回执并生成只读快照；复用同一项目统筹 G 的 `PUBLIC_QA_BATCH_READONLY` 回合比对当前已回传文章，不重启 W/R。

可见角色会话、文章路径隔离与 Git 工作树是不同概念。当前默认是 `CONTINUOUS_CAMPAIGN_MAIN_SESSION` 加 `MAIN_SESSION_PATH_ISOLATED`：可见 W/R 会话可以跨普通文章复用，R 仍独立于 W，且每篇必须只读写自己的 `articles/<article_id>/`。全项目只登记一个可复用的 `CAMPAIGN_GATEKEEPER`。Git 工作树只在该文章记录 `worktree_dispatch_decision` 且理由为 `TRUE_CONCURRENT_WRITE`、`HIGH_RISK_REWRITE_OR_ROLLBACK` 或 `OWNER_REQUESTED_GIT_ISOLATION` 时允许；槽位、容量或 token 节省不是理由。CLI 只可执行文件读取、JSON 校验、哈希、脚本测试和版本控制等确定性本地操作，不能代替独立角色。

每次交接前运行 `scripts/harnessctl.py check --workspace <campaign-root>`。W 将最终稿交给 R 前，仍以含 `campaign.json` 的内容项目根运行 `check-review-ready --workspace <campaign-root> --article-contract articles/<article_id>/context/article-contract.json --review-index articles/<article_id>/reviews/review-index.json --article-package articles/<article_id>/article-package.json`：它校验当前 article contract、review index、实际文件哈希、共享包引用/本篇差异的结构边界、固定载荷、topic-slot 对齐、精确 CTA 和伴随投影可验证性，防止 R 把 token 花在缺件或过期交付包上；它不是质量评分或批准。无共享引用时不要求空缓存。`TOPIC_EVIDENCE_CONFLICT` 只能在同一已登记 G 的可见 `TOPIC_EVIDENCE_DECISION` 记录为 `KEEP_SLOT` / `NARROW_WITHIN_SLOT` 后恢复可审，且不新增 R。当前 schema-2.13 文章包还运行 `check-article-package`、`check-handoff-manifest`，并在 G 批量验收时运行 `check-batch-gate`；它们只验证结构、哈希、正确审稿路线、人工发布映射和声明保真，不评分文章质量。标准路线要求绑定 evidence pack 的一次完整 R 审稿，默认审 HTML 主投影，只有 `COMPANION_DUAL_READ_REQUIRED` 才人工双读 Markdown；不要求虚构一份提前研究审。增强路线同时要求研究审和完整审。公开回传先运行 `check-public-return-receipt`。任何修复仍回到同一 R，但有完整变更摘要时优先作边界明确的 R-Δ，而不是为单一已定位问题机械重审全文。
