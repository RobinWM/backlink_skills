...
- [ ] The `handoff/handoff-manifest.json` hashes bind the approved canonical article, metadata, evidence pack, visual manifest, **HTML and Markdown visual payloads**, requirements traceability and review index. For current packages, the final full R review normally covers the completed payload pair; a bounded same-R delta reviews a documented repair and its direct dependencies, while an unclear or scope-changing repair returns to full R. G later checks the batch contract row only; it does not add a per-article editorial review.
