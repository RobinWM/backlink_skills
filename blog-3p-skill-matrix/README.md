...
- **Regional SERP:** target-region search results establish reader intent and naturally used semantic variants. For every target language, Google Trends compares English seeds/candidates only and records the English-concept-to-regional-SERP map; it finds low-base, high-momentum global topic candidates only and is never presented as local-demand evidence.
...
# For a current schema-2.4 article package (schema 1.3), verify its canonical
